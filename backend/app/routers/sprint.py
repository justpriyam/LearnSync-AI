...
def list_sprints(
    ...
    """List all sprint plans for the current user."""
    return (
        db.query(SprintPlan)
...
def generate_sprint(
    req: SprintGenerateRequest,
    background_tasks: BackgroundTasks,
    db: Session = Depends(get_db),
    user_id: str = Depends(get_current_user),
):
    syl_doc = db.query(Document).filter(
        Document.id == req.syllabus_document_id,
    ...
    pyq_doc = db.query(Document).filter(
        Document.id == req.pyq_document_id,
        Document.user_id == user_id,
    ).first()
    ...
    course = db.query(Course).filter(
        Course.document_id == req.syllabus_document_id,
...
def get_sprint(
    ...
    sprint = db.query(SprintPlan).filter(
        SprintPlan.id == sprint_id,
